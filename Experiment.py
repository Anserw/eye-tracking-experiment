...
class Experiment():
    ...
    def start_from_i(self, index, is_male, age):
        get_gender = lambda b: 'male' if b else 'female'
        userID = get_gender(is_male) + str(age) + datetime.now().strftime('_%m%d%H%M%S')
    ...
    def start(self, is_male=True, age=22):
        self.start_from_i(0, is_male, age)
